...
public class JwtService {
  ...
  // Constructor alternativa para pruebas (permite inyectar un Clock custom).
  ...
  public String generateRefreshTokens(UserAccount user) {
   ...
   * @param User usuario autenticado
   * @return ttlSeconds tiempo de vida en segundos
   * @return JWT firmado
   */
   ...
   * @param claims claims extrídos del JWT
  ...
  private void validateStandardClaims(JWTClaimsSet claims) {
    ...
    if (exp == null || exp.toInstant().isBefore(now.minusSeconds(30))) { // 30 clock skew
  ...
  /*
   * Convierte el set de roles (enum) a lista de string para el claim "roles".
   */
