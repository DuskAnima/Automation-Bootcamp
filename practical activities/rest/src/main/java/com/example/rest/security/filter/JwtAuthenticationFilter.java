 ...
 * - Si no hay tokens, deja pasar la request (puede ser endpoint público).
 * 
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
   ...
   * @param filterChain cadena de filtros de Spring Scurity
   */
  ...
  protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain)
    ...
    if (header != null && header.startsWith("Bearer ")) {
      ...
      try {
        ...
        // Si el token es inválido, responde 401 Unauthorized en JSON y detiene la cadena.bookController
    ...
    // Si no hay token, o es endpoints público, continúa la cadena normalmente.
